...
/// 命名颜色条目——(名称, R, G, B)。
type NamedEntry = (&'static str, u8, u8, u8);

/// 全部 CSS 命名颜色 + transparent。
///
/// 按 ASCII 排序，便于二分查找（线性扫描也足够快，~150 条）。
pub const NAMED_COLORS: &[NamedEntry] = &[
...
/// 反向查找——RGB 值 → 名称。
///
/// 使用 `COLOR_MATCH_TOLERANCE` 容差匹配，适应 f64 浮点运算后的微小偏差。
...
/// 正向查找返回 RGBA 元组（transparent 的 alpha = 0.0）。
pub fn lookup_rgba(name: &str) -> Option<(f64, f64, f64, f64)> {
    lookup(name).map(|(r, g, b)| {
        let alpha = if is_transparent(name) { 0.0 } else { 1.0 };
        (r as f64, g as f64, b as f64, alpha)
    })
}
