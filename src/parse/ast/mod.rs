...
/// mixin 引用数据——`meta.get-mixin` 返回的值的内部表示。
/// 存储 mixin 名称、来源模块命名空间和 mixin 定义的快照。
#[derive(Debug, Clone)]
pub struct MixinRefData {
    /// mixin 名称。
    pub name: String,
    /// 来源模块命名空间（`None` 表示全局）。
    ...
    /// mixin 体节点列表。
    pub body: Vec<Node>,
    /// mixin 定义时捕获的命名空间名称列表（用于后续查找）。
    pub captured_ns_keys: Vec<String>,
}

impl PartialEq for MixinRefData {
    /// 引用相等性：名称 + 模块 + 参数 + 体全等。
    /// redefined 后 body 不同 → 不相等。
