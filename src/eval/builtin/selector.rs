...
pub fn call(name: &str, args: &[Value]) -> Result<Option<Value>> {
    match name {
        ...
        "selector-unify" => match args {
            [Value::String(a, _), Value::String(b, _)] => {
                if a.contains(b.as_str()) { Ok(Some(Value::String(a.clone(), false))) }
                else if b.contains(a.as_str()) { Ok(Some(Value::String(b.clone(), false))) }
                else { Ok(Some(Value::String(format!("{a}{b}"), false))) }
            }
