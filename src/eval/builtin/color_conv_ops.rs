...
pub(crate) fn convert_space(c: &Color, target_space: &str) -> Result<Value> {
    ...
    match target_space {
        "rgb" => {
            ...
            Ok(Value::Color(Color::with_rgb(r255, g255, b255, c.a, ColorSpace::Rgb, ColorOutput::Auto)))
        }
        "srgb" => {
            Ok(make_color(ColorSpace::Srgb, [r, g, b], c.a, ColorOutput::Auto))
        }
        "srgb-linear" => {
            let (rl, gl, bl) = color_conv::srgb_to_linear_srgb(r, g, b);
            Ok(make_color(ColorSpace::SrgbLinear, [rl, gl, bl], c.a, ColorOutput::Auto))
        }
        "hsl" => {
            let (h, s, l) = match c.space {
                ColorSpace::Hsl => (c.channels[0], c.channels[1], c.channels[2]),
                ColorSpace::Hwb => hwb_to_hsl_via_color(c.channels[0], c.channels[1], c.channels[2]),
                _ => Evaluator::rgb_to_hsl(c.legacy_rgb[0], c.legacy_rgb[1], c.legacy_rgb[2]),
            };
            Ok(Value::Color(Color::with_hsl(h, s, l, c.a, ColorOutput::Auto, c.legacy_rgb)))
        }
        "hwb" => {
            let (h, w, bk) = match c.space {
                ColorSpace::Hwb => (c.channels[0], c.channels[1], c.channels[2]),
                _ => {
        ...
        "lab" => {
            ...
            Ok(make_color(ColorSpace::Lab, [l, a, b_lab], c.a, ColorOutput::Auto))
        }
        "lch" => {
            let (l, c_lch, h) = color_conv::srgb_to_lch(r, g, b);
            Ok(make_color(ColorSpace::Lch, [l, c_lch, h], c.a, ColorOutput::Auto))
        }
        "oklab" => {
            let (l, a, b_ok) = color_conv::srgb_to_oklab(r, g, b);
            Ok(make_color(ColorSpace::Oklab, [l, a, b_ok], c.a, ColorOutput::Auto))
        }
        "oklch" => {
            let (l, c_ok, h) = color_conv::srgb_to_oklch(r, g, b);
            Ok(make_color(ColorSpace::Oklch, [l, c_ok, h], c.a, ColorOutput::Auto))
        }
        "display-p3" => {
            let (rp, gp, bp) = color_conv::srgb_to_display_p3(r, g, b);
            Ok(make_color(ColorSpace::DisplayP3, [rp, gp, bp], c.a, ColorOutput::Auto))
        }
        "display-p3-linear" => {
            let (rl, gl, bl) = color_conv::srgb_to_linear_srgb(r, g, b);
            Ok(make_color(ColorSpace::DisplayP3Linear, [rl, gl, bl], c.a, ColorOutput::Auto))
        }
        "a98-rgb" => {
            let (rp, gp, bp) = color_conv::srgb_to_a98_rgb(r, g, b);
            Ok(make_color(ColorSpace::A98Rgb, [rp, gp, bp], c.a, ColorOutput::Auto))
        }
        "prophoto-rgb" => {
            let (rp, gp, bp) = color_conv::srgb_to_prophoto(r, g, b);
            Ok(make_color(ColorSpace::ProphotoRgb, [rp, gp, bp], c.a, ColorOutput::Auto))
        }
        "rec2020" => {
            let (rp, gp, bp) = color_conv::srgb_to_rec2020(r, g, b);
            Ok(make_color(ColorSpace::Rec2020, [rp, gp, bp], c.a, ColorOutput::Auto))
        }
        "xyz" | "xyz-d65" => {
            let (x, y, z) = color_conv::srgb_to_xyz_d65(r, g, b);
            Ok(make_color(ColorSpace::XyzD65, [x, y, z], c.a, ColorOutput::Auto))
        }
        "xyz-d50" => {
            let (x_d65, y_d65, z_d65) = color_conv::srgb_to_xyz_d65(r, g, b);
            let (x, y, z) = color_conv::xyz_d65_to_xyz_d50(x_d65, y_d65, z_d65);
            Ok(make_color(ColorSpace::XyzD50, [x, y, z], c.a, ColorOutput::Auto))
        }
...
pub(crate) fn space_to_srgb_f64(space: ColorSpace, channels: [f64; 3], legacy_rgb: [f64; 3]) -> (f64, f64, f64) {
    use super::color_conv;
    match space {
        ColorSpace::Rgb => (legacy_rgb[0] / RGB_MAX, legacy_rgb[1] / RGB_MAX, legacy_rgb[2] / RGB_MAX),
        ColorSpace::Srgb => (channels[0], channels[1], channels[2]),
        ColorSpace::SrgbLinear => color_conv::linear_srgb_to_srgb(channels[0], channels[1], channels[2]),
        ColorSpace::DisplayP3 => color_conv::display_p3_to_srgb(channels[0], channels[1], channels[2]),
        ColorSpace::DisplayP3Linear => color_conv::linear_srgb_to_srgb(channels[0], channels[1], channels[2]),
        ColorSpace::A98Rgb => color_conv::a98_rgb_to_srgb(channels[0], channels[1], channels[2]),
        ColorSpace::ProphotoRgb => color_conv::prophoto_to_srgb(channels[0], channels[1], channels[2]),
        ...
        ColorSpace::XyzD50 => {
            let (x_d65, y_d65, z_d65) = color_conv::xyz_d50_to_xyz_d65(channels[0], channels[1], channels[2]);
...
pub(crate) fn color_name(c: &Color) -> String {
    format!("#{:02x}{:02x}{:02x}", c.legacy_rgb[0].round() as u8, c.legacy_rgb[1].round() as u8, c.legacy_rgb[2].round() as u8)
}

/// 从空间和通道创建 Color（含 sRGB 近似值）。
pub(crate) fn make_color(space: ColorSpace, channels: [f64; 3], alpha: f64, output: ColorOutput) -> Value {
