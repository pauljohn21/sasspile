...
impl Evaluator {
    pub(crate) fn eval_if(branches: &[(Value, Vec<Node>)], else_body: &Option<Vec<Node>>, env: &Env) -> Result<(Vec<CssNode>, Env)> {
    ...
    pub(crate) fn eval_for(var: &str, from: &Value, to: &Value, inclusive: bool, body: &[Node], env: &Env) -> Result<(Vec<CssNode>, Env)> {
...
Ok((css, current_env))
    ...
    pub(crate) fn eval_each(vars: &[String], list: &Value, body: &[Node], env: &Env) -> Result<(Vec<CssNode>, Env)> {
