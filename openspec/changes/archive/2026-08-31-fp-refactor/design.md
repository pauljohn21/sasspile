...
1. `Env` 被 22 处直接字段赋值（跨 6 个文件：`module.rs` 8 处、`rule.rs` 6 处、`value/mod.rs` 2 处、`mod.rs` 2 处、`module_helpers.rs` 3 处、`mixin.rs` 2 处）
2. `env.clone()` 5 处（`mixin.rs` 2 处、`mod.rs` 1 处、`module.rs` 1 处），其中 1 处是 `@content` 上下文快照（允许保留）
...
7. `sasspile-macros` proc-macro crate（258 行）仅为生成三张字符串映射表，增加编译依赖
8. 后处理 `apply_extends`/`hoist_css_imports` 用 `&mut` 就地修改
9. 339 处非 env clone() 中大部分是 Value clone（Sass 语义要求值可多次引用，但存在冗余 clone）

测试基线：202/202 + sass-spec 2828/5362 = 53% + ep_full 121/121，重构必须保持基线不回归。

## Goals / Non-Goals

**Goals:**
- 所有 `Env` 字段修改通过 builder 方法，消除 22 处直接赋值
- `eval_node` match arm 全部委托独立函数，零内联逻辑
- 后处理改为纯函数 `Vec<CssNode> -> Vec<CssNode>`
- 文件按功能域组织，不为行数硬切
- 删除 `sasspile-macros` crate，内建函数注册内联到各模块
- `env.clone()` 仅保留 `@content` 上下文快照（1 处）
- `Evaluator` 空壳消除，所有方法改为自由函数
- 消除明显冗余的 Value clone（可 move 的值不 clone）
...
- 不引入新的外部依赖
- 不追求零 Value clone（Sass 语义需要值拷贝）

## Decisions

### D1: Env builder 方法补齐 + 作用域快照机制

**选择：补齐 builder 方法 + `enter_scope`/`exit_scope` 对**

补齐缺失的 builder 方法：`with_depth`、`with_plain_css`（已存在但未使用）、`with_loaded_modules`、`with_extends`、`with_module_cache`（已存在）、`with_namespaces`、`with_pending_config`、`with_global_write`。
...
### D2: `eval_node` 分发——保持 match + Evaluator 空壳消除

**选择：保持 match 分发 + 每个 arm 委托独立自由函数**
...
**Evaluator 空壳消除**：`Evaluator` 结构体零字段，所有 `impl Evaluator` 方法改为 `pub(crate) fn`，删除 `Evaluator` 类型，`evaluate`/`evaluate_with_env` 成为 `eval/mod.rs` 的自由函数。
...
`evaluate()` 入口改为链式：
```rust
let (css, final_env) = eval_nodes(&ast.nodes, Env::default())?;
let extends = final_env.get_extends().to_vec();
let css = if !extends.is_empty() {
    apply_extends(css, &extends)
} else {
    css
};
...
**选择：消除明显冗余 clone，不追求零 clone**

Sass 语义要求值可被多次引用（如列表中的元素可被多次求值），因此 Value clone 是必要的。但审计发现以下冗余模式需消除：
- 同一值在同一函数内 clone 多次（可 move 的值被 clone）
- 可用 `&Value` 传引用但用 clone
- 可返回 `Value`（move）但先 clone 再 move

**不消除的场景**：
- `@content` 上下文快照的 `env.clone()`（1 处，唯一例外）
- 值需要在多处保留引用时的 clone（Sass 语义要求）
...
- [Evaluator 空壳消除导致 API 变化] → `Evaluator` 是 `pub` 但零字段，用户不直接使用；`lib.rs` 的 `pub use eval::Evaluator` 改为 `pub use eval::evaluate`
