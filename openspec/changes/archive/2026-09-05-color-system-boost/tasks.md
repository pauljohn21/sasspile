...
- [x] 2.1 在 `color_parse.rs` 的 `parse_color_space()` 中添加单参数 `Value::Color` 透传
- [x] 2.2 在 `color_parse.rs` 的 `flatten_space_list` 和参数解析中识别 `none` 关键字 → `f64::NAN`
- [x] 2.3 在 `color.rs` 的 `hsl()`/`hsla()`/`hwb()` 创建函数中解析 `none` 参数
- [x] 2.4 在 `display.rs` 序列化中检测 `f64::NAN` → 输出 `none`
- [x] 2.5 在 `color_conv_ops.rs` 的 `space_to_srgb_f64` 转换中将 `NAN` 替换为 0.0

## 3. Legacy 同空间转换规范化

- [x] 3.1 在 `color_conv_ops.rs` 的 `convert_space()` 中：同空间转换直接返回（legacy 规范化由 display.rs Auto 输出处理）
- [x] 3.2 HWB Auto 输出经过 `hwb_to_hsl_inline` 规范化为 HSL 格式
- [x] 3.3 RGB→RGB 和 HSL→HSL 同空间直接返回原值
- [x] 3.4 现代空间同空间转换保持直接返回（Lab→Lab、Oklch→Oklch 等）

## 4. color.to-space 精度修复

- [x] 4.1 验证 `hwb_to_srgb_f64` 精度：对照 spec `hwb(0deg 20% 30%) → hsl(0, 55.5555555556%, 45%)`
- [x] 4.2 修复 `convert_space` 中 HWB 输出格式：HWB→其他空间时输出 HSL 而非 HWB
...
- [x] 6.1 在 `color.rs` 的 `invert` 中添加现代空间分派：oklch → hue+180，oklab → 转 oklch 反转
- [ ] 6.2 实现独立函数 `invert_oklch(c)` — hue + 180
- [ ] 6.3 实现独立函数 `invert_oklab(c)` — 转 oklch 反转 hue
- [ ] 6.4 实现独立函数 `invert_lab(c)` — 转 lch 反转 hue
- [x] 6.5 在 `color.rs` 的 `grayscale` 中添加现代空间分派（legacy 已输出 RGB Auto）
...
- [x] 8.1 在 `color_fmt.rs` 的 `format_num` 中确保最多 10 位小数，去除尾随零
- [x] 8.2 验证 HSL 输出百分比精度：`55.5555555556%` 格式（10 位小数）
- [ ] 8.3 验证 oklch hue 输出带 `deg` 后缀
- [ ] 8.4 验证 lch/oklch chroma=0 时 hue 输出为 `none`
- [x] 8.5 验证 HWB 颜色 Auto 输出格式（应为 HSL 而非 HWB）

## 9. 测试验证

- [x] 9.1 运行 `cargo test --test sass_spec_full -- --nocapture test_core_functions_subdirs`，确认通过率提升
- [x] 9.2 运行核心测试：`cargo test --test compile_test` + `cargo test --test ep_full`
- [x] 9.3 运行全量 sass-spec 统计：`cargo test --test sass_spec_full -- --nocapture test_sass_spec_full_stats`
- [x] 9.4 `codegraph sync`
- [x] 9.5 提交（等用户确认后推送）
