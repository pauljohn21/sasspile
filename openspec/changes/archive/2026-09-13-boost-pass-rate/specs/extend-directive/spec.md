...
### Scenario: @extend with simple selector
- **WHEN** `.a { color: red; } .b { @extend .a; }` is compiled
- **THEN** output contains both `.a` and `.b` with `color: red`

### Scenario: @extend within @media
- **WHEN** `@media screen { .a { color: red; } .b { @extend .a; } }` is compiled
- **THEN** `.b` extends `.a` only within the same `@media` scope
