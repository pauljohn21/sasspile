...
- [ ] 1.1 在 `Env` impl 中补齐缺失的 builder 方法：`with_depth`、`with_loaded_modules`、`with_extends`、`with_namespaces`
- [ ] 1.2 `eval/mod.rs:274` — `content_env.current_selector = env.current_selector.clone()` 改为 `content_env.with_selector(...)`
- [ ] 1.3 `eval/module.rs:46-50` — `env.depth =`/`env.plain_css =`/`env.loaded_modules =` 三个直接赋值改为 builder 方法
- [ ] 1.4 `eval/module.rs:125-127` — `env.base_path =`/`env.depth =`/`env.plain_css =` 改为 builder 方法
- [ ] 1.5 `eval/module.rs:130-131` — `final_env.base_path = saved`/`final_env.depth = saved` 改为 builder 方法
- [ ] 1.6 `eval/rule.rs:129-134` — 6 处 `return_env.xxx = saved_xxx` 改为 builder 方法或 `exit_scope`
- [ ] 1.7 `eval/module_helpers.rs:167-169` — `env.loaded_modules =`/`env.extends =`/`env.module_cache =` 改为 builder 方法
- [ ] 1.8 `eval/value/mod.rs` — `env.global_writes.insert` 和 `env.namespaces.insert` 改为 builder 方法
- [ ] 1.8 验证：`cargo test --test compile_test --test stage_test --test ast_test --test common_test` 全通过

## 2. eval_rule 作用域管理重构

- [ ] 2.1 在 `Env` 上实现 `enter_scope()` — 创建子作用域 Env（共享 Rc 字段）
- [ ] 2.2 在 `Env` 上实现 `exit_scope(child_env)` — 从子作用域提取传播字段（命名空间变量、global writes、新增 mixin/function）合并回父作用域
- [ ] 2.3 重写 `eval_rule` 用 `enter_scope`/`exit_scope` 替代手动 save/restore 6 个 HashMap
- [ ] 2.4 验证：`cargo test --test compile_test --test eval_test` 全通过

## 3. env.clone 消除

- [ ] 3.1 `eval/mixin.rs:103` — `bind_params` 中的 `env.clone()` 改为 move 或 `&Env`
- [ ] 3.2 `eval/mixin.rs:160` — `call_user_function` 中的 `env.clone().incr_depth()` 改为 move 语义
- [ ] 3.3 `eval/module.rs:164` — `call_module_function` 中的 `env.clone()` 改为 move 语义
- [ ] 3.4 验证 `@content` 的 `env.clone()`（`mixin.rs:62`）保留——这是唯一允许的例外
- [ ] 3.5 验证：`cargo test --test compile_test --test eval_test` 全通过
...
- [ ] 4.10 验证：`cargo test --test compile_test --test eval_test` 全通过
...
- [ ] 7.4 从 `control_flow.rs` 移出 `unit_conversion_factor` 到 `eval/value_ops.rs`
...
- [ ] 7.8 验证：`cargo test --test compile_test --test eval_test` 全通过
...
- [ ] 10.1 `cargo fmt`
- [ ] 10.2 `cargo clippy --workspace -- -W clippy::pedantic` 零警告
- [ ] 10.3 `cargo test --test compile_test --test stage_test --test ast_test --test common_test` — 66/66 通过
- [ ] 10.4 `cargo test --test bs_spec -- --nocapture` — 15/15 通过
- [ ] 10.5 `cargo test --test ep_full -- --nocapture` — 121/121 通过（约 38 秒）
- [ ] 10.6 `RUST_LOG="sass_spec_full=info,sasspile=warn" cargo test --test sass_spec_full -- --nocapture` — 2828 基线不回归
- [ ] 10.7 `cargo tree` 确认无 `sasspile-macros`/`syn`/`quote`/`proc-macro2` 依赖
