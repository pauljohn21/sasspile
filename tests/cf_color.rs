        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() { collect_hrx(&path, files); }
                ...
                if let Ok(meta) = std::fs::metadata(&path) {
                    if meta.len() < 100_000 { files.push(path); }
                }
            }
        }
    }
...
fn color_error_patterns() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../sass-spec-main/spec/core_functions/color");
                ...
                match sasspile::compile_expanded(input) {
                    Ok(actual) => {
                        if actual.trim() == expected.trim() {
                            ...
                            let a = actual.trim();
                            let e = expected.trim();
                            let key = if a.is_empty() { "empty".to_string() }
                                else if a.lines().next() != e.lines().next() { "first_line".to_string() }
