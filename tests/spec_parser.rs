//! Spec tests for `sass-spec/spec/parser/` domain. Uses OTel Metrics + Trace.
...
#[test] fn test_parser_otel() {
    let label = "spec_parser"; tracing_init::init_otel(label); tracing_init::init_metrics(label);
    let dir = spec_root().join("parser");
    if !dir.exists() { tracing::warn!(stage="spec_test", domain="parser", "dir not found, skipping"); tracing_init::shutdown_metrics(); tracing_init::shutdown_otel(); return; }
    let mut runner = spec_otel_runner::SpecOtelRunner::new("parser");
