//! Spec tests for `sass-spec/spec/expressions/` domain. Uses OTel Metrics + Trace.
...
#[test] fn test_expressions_otel() {
    let label = "spec_expressions"; tracing_init::init_otel(label); tracing_init::init_metrics(label);
    let dir = spec_root().join("expressions");
    if !dir.exists() { tracing::warn!(stage="spec_test", domain="expressions", "dir not found, skipping"); tracing_init::shutdown_metrics(); tracing_init::shutdown_otel(); return; }
    let mut runner = spec_otel_runner::SpecOtelRunner::new("expressions");
