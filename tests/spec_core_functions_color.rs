//! Spec tests for `sass-spec/spec/core_functions/color/` domain. Uses OTel Metrics + Trace.
...
#[test] fn test_core_functions_color_otel() {
    let label = "spec_core_functions_color"; tracing_init::init_otel(label); tracing_init::init_metrics(label);
    let dir = spec_root().join("core_functions/color");
    if !dir.exists() { tracing::warn!(stage="spec_test", domain="core_functions_color", "dir not found, skipping"); tracing_init::shutdown_metrics(); tracing_init::shutdown_otel(); return; }
    let mut runner = spec_otel_runner::SpecOtelRunner::new("core_functions_color");
