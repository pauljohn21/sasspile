//! HSL/Lab/Lch/Oklab/Oklch units 差异精准诊断。
mod hrx_support;
...
fn compile_case(case: &HrxCase, spec_root: &Path, hrx_dir: &Path, hrx_stem: &str) -> Result<String, String> {
    let tmp_dir = std::env::temp_dir().join(format!("cfs-units-{}", std::process::id()));
...
fn main_diag_full(subdir: &str, max: usize) {
...
#[test]
fn hsl_detail() { main_diag_full("core_functions/color/change", 250); }
