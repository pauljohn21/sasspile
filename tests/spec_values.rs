//! Spec tests for `sass-spec/spec/values/` domain. Uses OTel Metrics + Trace.
...
#[test] fn test_values_otel() {
    let label = "spec_values"; tracing_init::init_otel(label); tracing_init::init_metrics(label);
    let dir = spec_root().join("values");
    if !dir.exists() { tracing::warn!(stage="spec_test", domain="values", "dir not found, skipping"); tracing_init::shutdown_metrics(); tracing_init::shutdown_otel(); return; }
    let mut runner = spec_otel_runner::SpecOtelRunner::new("values");
