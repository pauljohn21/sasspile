//! Spec tests for `sass-spec/spec/callable/` domain. Uses OTel Metrics + Trace.
...
#[test] fn test_callable_otel() {
    let label = "spec_callable"; tracing_init::init_otel(label); tracing_init::init_metrics(label);
    let dir = spec_root().join("callable");
    if !dir.exists() { tracing::warn!(stage="spec_test", domain="callable", "dir not found, skipping"); tracing_init::shutdown_metrics(); tracing_init::shutdown_otel(); return; }
    let mut runner = spec_otel_runner::SpecOtelRunner::new("callable");
